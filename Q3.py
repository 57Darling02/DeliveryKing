...
# 效益函数
def benefit(order_num, time):
    return 5*order_num/time
...
allstatus = []  # 放本阶段包含的所有
opt_status = {"time": 0, "stage": 0, "ReceviedOrder": set(), "active": True, "path": [],"benefit":-M}
...
while True:
    ...
    for prestage_statu in stage[n - 1]:
        if not prestage_statu.get("active"):
            continue
        elif prestage_statu.get("benefit")>opt_status.get("benefit"):
            continue
        ...
        for add_index in canadd:
            ...
            temp_benefit = benefit(n,newtime)
            if newtime < limit_time and temp_benefit >= prestage_statu.get("benefit"):
                newstatus = {"time": newtime, "estimatedtime": estimatedtime, "stage": n,
                             "ReceviedOrder": new_get_index, "active": True, "path": newpath,"benefit": temp_benefit}
            else:
                newstatus = {"time": newtime, "estimatedtime": estimatedtime, "stage": n,
                             "ReceviedOrder": new_get_index, "active": False, "path": newpath,"benefit": temp_benefit}
            # 找最优
            itercount += 1
            temptime = GetTime(start_time, opt_status, OG, itercount)

            if temp_benefit >= opt_status.get("benefit"):
                opt_status = newstatus
                optresult.append(f"效益:{temp_benefit},接单数{n},迭代次数{itercount},运行至此耗时{round(temptime, 2)}s")
